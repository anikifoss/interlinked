...
import requests.StreamHeaders
import requests.Util
import requests.UtilBypass
import requests.TimeoutException
import requests.UnknownHostException
import requests.RequestsException
import requests.InvalidCertException
import requests.RequestFailedException
import requests.StatusMessages
    ...
    def readBytesThrough[T](f: java.io.InputStream => T): T = {
      ...
      val httpClient: HttpClient =
        HttpClient
          ...
          .sslContext(
            if (cert != null)
              UtilBypass.clientCertSSLContext(cert, verifySslCerts)
            else if (sslContext != null)
              sslContext
            else if (!verifySslCerts)
              UtilBypass.noVerifySSLContext
            else
